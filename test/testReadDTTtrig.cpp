...
//  std::string m_dbConnection( "sqlite_file:trackerped.db");
  ...
  try {
    ...
    std::string tTrigToken(cline[1]);
    std::cout << tTrigToken << std::endl;
    svc->transaction().start(pool::ITransaction::READ);
    std::cout << "1" << std::endl;
    pool::Ref<DTTtrig> tTrig(svc,tTrigToken);
    std::cout << "2" << std::endl;
    std::cout << tTrig->version() << std::endl;
    std::cout << "3" << std::endl;
    std::cout << std::distance( tTrig->begin(), tTrig->end() ) << " data in the tTrig set" << std::endl;
    std::cout << "4" << std::endl;
    std::cout << "5" << std::endl;
